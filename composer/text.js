        ...
        const response = await axios.post(
            'https://openrouter.ai/api/v1/chat/completions',
            {
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'Твое имя Gecko. Ты умный AI помощник в Telegram боте. Отвечай кратко. Всегда говори что тебя зовут Gecko когда спрашивают о твоем имени.'
                    },
                    {
                        role: 'user',
                        content: prompt
...
composer.start(async (ctx) => {
    ...
    const keyboard = {
        reply_markup: {
            inline_keyboard: [[{ 
                text: '🦎 Go App', 
                url: 'https://t.me/GeckoGramRobot/GeckoGram'
            }]]
        }
    };
    ...
    try {
        await ctx.replyWithPhoto(
            'https://github.com/MatveyVue/GeckoNumbers/blob/main/Gecko.jpg?raw=true',
            { caption: msg, parse_mode: 'Markdown', ...keyboard }
        );
    } catch {
...
composer.on('text', async (ctx) => {
    ...
    if (!isPrivateChat) {
        ...
        const botUsername = ctx.botInfo?.username || 'GeckoGramRobot';
        ...
        if (!cleanText) {
            return ctx.reply('🦎 Да, это я! Gecko ваш помощник!');
        }
    }
    
    // Для приватных чатов или если бот упомянут - обрабатываем сообщение
    const processedText = isPrivateChat ? text : text.replace(`@${ctx.botInfo?.username || 'GeckoGramRobot'}`, '').trim();
    ...
    if (lowerText.includes('как тебя зовут') || lowerText.includes('твое имя') || 
        ...
        return ctx.reply('🦎 Меня зовут Gecko!');
    }
